...
/-- A pointed coalgebra -/
structure StateCofix (α : TypeVec.{u} n) : Type (u+1) where
  corec ::
  {σ : Type u}
  (dest' : σ → F (α ::: σ))
  (s : σ)
...
namespace StateCofix
variable {self : StateCofix F α}
...
def dest (self : StateCofix F α) : F (α ::: self.σ) := self.dest' self.s
...
def toCofix (self : StateCofix F α) : Cofix F α :=
  Cofix.corec self.dest' self.s

def ofCofix (fix : Cofix F α) : StateCofix F α :=
...
theorem ofCofix_toCofix (self : StateCofix F α) : ofCofix (toCofix self) ~ self := by
  simp only [toCofix, ofCofix]
...
variable (F) (α) in
def StateCofixFinal := Quot (StateCofix.Bisim (F := F) (α := α))
...
def corec {β} (f : β → F (α ::: β)) (init : β) : StateCofixFinal F α :=
  Quot.mk _ <| StateCofix.corec f init
...
theorem ofCofix_toCofix (self : StateCofixFinal F α) : ofCofix (toCofix self) = self := by
  ...
  apply Quot.sound
  simpa using StateCofix.ofCofix_toCofix _
...
def dest (self : StateCofixShrunk F α) : F (α ::: (StateCofixShrunk F α)) :=
  ...
  have h := by
    ...
    rintro a b h_bisim
    unfold StateCofix.Bisim at h_bisim
    rcases h_un : a.union b with ⟨un, d'⟩
    simp only [StateCofix.dest, f]
